...
fleet/
  ...
  +-- difficulty.js     <- Per-difficulty config (fleet composition, island settings)
  ...
  +-- tests/
      ...
      +-- difficulty.test.js <- 9 tests for Difficulty module
```
...
**board.js**: Exports a Board class. Manages a 10x10 grid. Cell types: empty, ship, hit, miss, island. Methods: `placeShip()`, `placeIslands()`, `processShot()`, `allShipsSunk()`, `getGrid()`, `placeShipsRandomly()`. Does not know about rendering.
...
**difficulty.js**: Exports `DIFFICULTIES` array and `DIFFICULTY_CONFIGS` object. Each config specifies the ship list (name + size) and island settings (count + style: clustered/scattered/none) for that difficulty level.
