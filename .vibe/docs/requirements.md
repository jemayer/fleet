...
- Each difficulty varies fleet composition and board terrain:
  - Easy: 6 ships (adds Destroyer II), 4 clustered 2x2 island groups — high target density
  - Medium: 5 ships, open ocean — classic Battleship (unchanged)
  - Hard: 4 ships (no Carrier), 8 scattered single-cell islands — low target density
- Islands are visible obstacles that block ship placement and shots
- AI targeting intelligence also scales: random (Easy), hunt-adjacent (Medium), axis-lock (Hard)
- AI places its own ships randomly (valid placement, respecting islands)
